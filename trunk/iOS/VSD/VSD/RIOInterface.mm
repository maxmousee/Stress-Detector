					   ...
					   AudioBufferList				*ioData)
{
    ...
    else
    {
		...
		// Determine the high signal frequency by taking the magnitude squared and 
		// saving the bin which it resides in.
        int HFBin = -1;
		for (int i=0; i<n; i+=2)
        {
			float threshold = MagnitudeSquared(outputBuffer[i], outputBuffer[i+1]);
            //threshold set here
            //if (threshold >= THIS->thresholdBIAS)
            if (threshold >= 2)
            {
                HFBin = (i+1)/2;
                float highFrequency = HFBin*(THIS->sampleRate/bufferCapacity);
                if (highFrequency >= 18220) {
                    //check what data value is this
                    //printf("High Frequency present: %f Hertz \n", highFrequency);
                    //printf("threshold %f\n", threshold);
                    if ((highFrequency >= 18680) && (highFrequency <= 18737))
                    {
                        tmpData = 0;
                        //printf("Data = 0\n");
                        //printf("0 FREQ %f\n", highFrequency);
                    }
                    else if ((highFrequency >= 18908) && (highFrequency <= 18964))
                    {
                        tmpData = 1;
                        //printf("1 FREQ %f\n", highFrequency);
                        //printf("Data = 1\n");
                    }
                    else if((highFrequency >= 19139) && (highFrequency <= 19197))
                    {
                        tmpData = 2;
                        //printf("2 FREQ %f\n", highFrequency);
                        //printf("Data = 2\n");
                    }
                    else if((highFrequency >= 19377) && (highFrequency <= 19435))
                    {
                        tmpData = 3;
                        //printf("Data = 3\n");
                    }
                    else if((highFrequency >= 19620) && (highFrequency <= 19680))
                    {
                        tmpData = 4;
                        //printf("Data = 4\n");
                    }
                    else if((highFrequency >= 19870) && (highFrequency <= 19931))
                    {
                        tmpData = 5;
                        //printf("Data = 5\n");
                    }
                    else if((highFrequency >= 20126) && (highFrequency <= 20188))
                    {
                        tmpData = 6;
                        //printf("Data = 6\n");
                    }
                    else if((highFrequency >= 20390) && (highFrequency <= 20452))
                    {
                        tmpData = 7;
                        //printf("Data = 7\n");
                    }
                    else if((highFrequency >= 20659) && (highFrequency <= 20723))
                    {
                        tmpData = 8;
                        //printf("Data = 8\n");
                    }
                    else if((highFrequency >= 20937) && (highFrequency <= 21001))
                    {
                        tmpData = 9;
                        //printf("Data = 9\n");
                    }
                    else if ((highFrequency >= 18220) && (highFrequency <= 18280))
                    {
                        tmpData = 10;
                        //printf("START FREQ %f\n", highFrequency);
                        //printf("START Signal\n");
                    }
                    else if ((highFrequency >= 18470) && (highFrequency <= 18510))
                    {
                        tmpData = 11;
                        //printf("SPLIT FREQ %f\n", highFrequency);
                        //printf("SPLIT Signal\n");
                    ...
                    /*
                    if(threshold > 10 && THIS->thresholdBIAS >= 1.0) {
                        THIS->thresholdBIAS += 0.5;
                    }
                    else if(threshold < 10 && THIS->thresholdBIAS >= 1.0) {
                        THIS->thresholdBIAS -= 0.5;
                    }
                     */
                }
                ...
                if(tmpData != -1)
                {
                    [THIS->listener frequencyChangedWithValue:highFrequency:tmpData];
                }
            }
            /*
