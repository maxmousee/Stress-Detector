...
static void bluestein(const double x_data[], const int x_size[2], int xoffInit,
                      const double costab[8193], const double sintab[8193],
                      const double costabinv[8193], const double sintabinv[8193],
                      const creal_T wwc[15999], creal_T y[8000]);
                      ...
                      const creal_T wwc[15999], creal_T y[8000])
{
  ...
  int k;
  vsdTLS *vsdTLSThread;
  vsdTLSThread = emlrtGetThreadStackData();
  if (8000 > x_size[0]) {
    for (i = 0; i < 8000; i++) {
      y[i].re = 0.0;
      y[i].im = 0.0;
  ...
  r2br_r2dit_trig_impl(y, costab, sintab, vsdTLSThread->f0.fv);
  r2br_r2dit_trig(wwc, costab, sintab, vsdTLSThread->f0.dcv0);
  for (i = 0; i < 16384; i++) {
    vsdTLSThread->f0.b_fv[i].re = vsdTLSThread->f0.fv[i].re *
      vsdTLSThread->f0.dcv0[i].re - vsdTLSThread->f0.fv[i].im *
      vsdTLSThread->f0.dcv0[i].im;
    vsdTLSThread->f0.b_fv[i].im = vsdTLSThread->f0.fv[i].re *
      vsdTLSThread->f0.dcv0[i].im + vsdTLSThread->f0.fv[i].im *
      vsdTLSThread->f0.dcv0[i].re;
  }

  b_r2br_r2dit_trig(vsdTLSThread->f0.b_fv, costabinv, sintabinv,
                    vsdTLSThread->f0.fv);
  i = 0;
  for (k = 0; k < 8000; k++) {
    y[i].re = wwc[k + 7999].re * vsdTLSThread->f0.fv[k + 7999].re + wwc[k + 7999]
      .im * vsdTLSThread->f0.fv[k + 7999].im;
    y[i].im = wwc[k + 7999].re * vsdTLSThread->f0.fv[k + 7999].im - wwc[k + 7999]
      .im * vsdTLSThread->f0.fv[k + 7999].re;
    i++;
  }
}
                    ...
                    sintabinv[8193], emxArray_creal_T *y)
{
  ...
  for (ub_loop = 0; ub_loop < 2; ub_loop++) {
    sx[ub_loop] = (short)x_size[ub_loop];
    sz[ub_loop] = (short)x_size[ub_loop];
  }
  ...
  if (8000 > x_size[0]) {
    ...
    for (ub_loop = 0; ub_loop < loop_ub; ub_loop++) {
      for (i7 = 0; i7 < 8000; i7++) {
        y->data[i7 + y->size[0] * ub_loop].re = 0.0;
        y->data[i7 + y->size[0] * ub_loop].im = 0.0;
      }
    }
  }
...
#pragma omp for nowait
    ...
    for (k = 1; k <= ub_loop; k++) {
      xoff = (k - 1) * nrows;
      bluestein(x_data, x_size, xoff, costab, sintab, costab, sintabinv, wwc,
                vsdTLSThread->f1.rwork);
      for (xoff = 0; xoff < 8000; xoff++) {
        y->data[xoff] = vsdTLSThread->f1.rwork[xoff];
      }
    }
  }
}
